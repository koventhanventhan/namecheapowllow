    ...
    >
      ...
      <div className="p-6">
        ...
        <div className="mt-5 flex items-center justify-between border-t border-border/50 pt-4">
          <span className="text-xs text-muted-foreground font-medium">By {post.author || 'Admin'}</span>
...
export function BlogClient({ posts = [] }: { posts: BlogPost[] }) {
  const featuredPost = posts.length > 0 ? {
    ...posts[0],
    image: posts[0].coverImage || '/placeholder.jpg',
    category: 'Article',
    date: new Date(posts[0].publishedAt || posts[0].createdAt).toLocaleDateString(),
    machineDate: new Date(posts[0].publishedAt || posts[0].createdAt).toISOString(),
    readTime: '5 min read'
  } : null;
  const gridPosts = posts.slice(1).map(post => ({
    ...post,
    image: post.coverImage || '/placeholder.jpg',
    category: 'Article',
    date: new Date(post.publishedAt || post.createdAt).toLocaleDateString(),
    machineDate: new Date(post.publishedAt || post.createdAt).toISOString(),
    readTime: '5 min read'
  }));
                      ...
                      <li key={post.id}>
                        <Link href={`/blog/${post.slug}`} className="group flex gap-3 items-start">
                          ...
                          <div>
                            ...
                            <span className="text-xs text-muted-foreground mt-1 block"><time dateTime={new Date(post.publishedAt || post.createdAt).toISOString()}>{new Date(post.publishedAt || post.createdAt).toLocaleDateString()}</time></span>
                          </div>
                        </Link>
                      </li>
                    ))}
