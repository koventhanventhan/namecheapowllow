            ...
            <AnimatedSection key={post.id} delay={i * 100} className="group">
              <article className="h-full overflow-hidden rounded-2xl border border-border bg-card transition-all duration-300 hover:border-primary/30 hover:shadow-xl hover:shadow-primary/5">
                <Link href={`/blog/${post.slug}`} className="relative block aspect-[16/10] overflow-hidden">
                  <Image src={post.coverImage || '/placeholder.jpg'} alt={post.title} fill sizes="(max-width: 768px) 90vw, 33vw" className="object-cover transition-transform duration-700 group-hover:scale-105" />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/50 to-transparent" />
                  <Badge className="absolute bottom-4 left-4 border-0 bg-primary text-primary-foreground">Article</Badge>
                </Link>
                <div className="p-6">
                  <div className="flex items-center gap-4 text-xs text-muted-foreground">
                    <span className="flex items-center gap-1"><CalendarDays className="h-3.5 w-3.5" />{new Date(post.publishedAt || post.createdAt).toLocaleDateString()}</span>
                  ...
                  <div className="mt-5 flex items-center justify-between border-t border-border pt-4">
                    <span className="text-xs text-muted-foreground">By {post.author || 'Admin'}</span>
                    <Link href={`/blog/${post.slug}`} className="flex items-center gap-1 text-sm font-semibold text-primary">Read more <ArrowRight className="h-3.5 w-3.5" /></Link>
                  </div>
                </div>
              </article>
            </AnimatedSection>
